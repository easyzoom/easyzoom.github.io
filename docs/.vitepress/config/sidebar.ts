...
export const sidebar: DefaultTheme.Config['sidebar'] = {
  ...
  '/courses/linux/': getItems("courses/linux"),
  '/courses/linuxdev/': getItems("courses/linuxdev"),
  '/courses/kernel/': getItems("courses/kernel"),
  '/courses/uboot/': getItems("courses/uboot"),
  '/courses/mcu/': getItems("courses/mcu"),
  '/courses/fpga/': getItems("courses/fpga"),
  '/courses/c/': getItems("courses/c"),
  '/courses/cpp/': getItems("courses/cpp"),
  '/courses/python/': getItems("courses/python"),
  '/courses/rust/': getItems("courses/rust"),
  '/courses/ucos/': getItems("courses/ucos"),
  '/courses/freertos/': getItems("courses/freertos"),
  '/courses/zephyr/': getItems("courses/zephyr"),
  '/courses/ros2/': getItems("courses/ros2"),
  '/courses/openlib/': getItems("courses/openlib"),
  '/courses/docker/': getItems("courses/docker"),
  '/courses/shell/': getItems("courses/shell"),
  '/courses/git/': getItems("courses/git"),
  '/courses/nxp/': getItems("courses/nxp"),
  '/courses/rk/': getItems("courses/rk"),
  '/courses/nvidia/': getItems("courses/nvidia"),
  '/courses/ai-basics/': getItems("courses/ai-basics"),
  '/courses/ai-agent/': getItems("courses/ai-agent"),
  '/courses/vibe-coding/': getItems("courses/vibe-coding"),
}
 ...
 * 
 * courses/java/01-Java语法入门/01-xxx.md
 * 
 * @param path 扫描基础路径
 * @returns {DefaultTheme.SidebarItem[]}
 */
function getItems (path: string) {
  ...
  sync(`docs/${path}/*`, {
    ...
    // 2a. 分组根目录下的 .md（按文件名排序）
    sync(`docs/${path}/${groupName}/*.md`, {
      onlyFiles: true,
      objectMode: true,
    })
    ...
    // 2b. 一级子目录内的 .md（子目录名排序后，各目录内再按文件名排序）
    sync(`docs/${path}/${groupName}/*`, {
      onlyDirectories: true,
      objectMode: true,
    })
      .sort((a, b) => a.name.localeCompare(b.name, 'zh-CN'))
      .forEach(({ name: subName }) => {
        if (subName.startsWith('.')) return;
        const subArticles = sync(`docs/${path}/${groupName}/${subName}/*.md`, {
          onlyFiles: true,
          objectMode: true,
        }).sort((a, b) => a.name.localeCompare(b.name, 'zh-CN'));
        ...
        subArticles.forEach((article) => {
          const articleFile = matter.read(`${article.path}`);
          const { data } = articleFile;
          const base = article.name.replace('.md', '');
          subItems.push({
            text: data.title,
            link: `/${path}/${groupName}/${subName}/${base}`,
          });
          groupTotal += 1;
        });

        items.push({
          text: `${formatSidebarSubdirName(subName)} (${subItems.length}篇)`,
          items: subItems,
          collapsed: false,
        });
      });

    // 3.向前追加到分组（折叠判断仅基于本组文章数）
    groups.push({
      text: `${groupName.substring(groupName.indexOf('-') + 1)} (${items.length}篇)`,
      items: items,
      collapsed: items.length < groupCollapsedSize || groupTotal > titleCollapsedSize,
    });
  })
...
function addOrderNumber(groups) {
  for (let i = 0; i < groups.length; i++) {
    ...
    for (let j = 0; j < groups[i].items.length; j++) {
      const item = groups[i].items[j];
      if (item.link) {
        order += 1;
        item.text = `${getOrderNumberStyle(order)}${item.text}`;
        continue;
      }
      ...
      if (item.items) {
        for (let k = 0; k < item.items.length; k++) {
          order += 1;
          item.items[k].text = `${getOrderNumberStyle(order)}${item.items[k].text}`;
        }
      }
    }
  }
...
function getOrderNumberStyle(index: number) {
  if (index == 1) {
    return `<div class="text-color-red mr-[6px]" style="font-weight: 550; display: inline-block;">${index}</div>`;
  }
  if (index == 2) {
    return `<div class="text-color-orange mr-[6px]" style="font-weight: 550; display: inline-block;">${index}</div>`;
  }
  if (index == 3) {
    return `<div class="text-color-yellow mr-[6px]" style="font-weight: 550; display: inline-block;">${index}</div>`;
  }
  return `<div class="text-color-gray mr-[6px]" style="font-weight: 550; display: inline-block;">${index}</div>`;
}

function formatSidebarSubdirName(subName: string) {
